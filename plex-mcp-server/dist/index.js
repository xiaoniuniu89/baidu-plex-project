...
class PlexManagerServer {
    ...
    async handleAuditLibrary() {
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    if (codecs.includes("av1"))
                        issues.push("AV1 (Hardware Incompatible)");
                    if (codecs.includes("vc1"))
                        issues.push("VC-1 (Legacy/Heavy)");
                    if (codecs.includes("truehd"))
                        issues.push("TrueHD (Sync Drift Risk)");
                    if (codecs.includes("dts"))
                        issues.push("DTS-HD (Sync Drift Risk)");
                    if (issues.length > 0) {
            ...
            if (!auditResults.includes("NEEDS UPDATE"))
                auditResults += "✅ All media is optimized!";
            return {
                content: [
                    {
                        type: "text",
    ...
    async handleTranscodeVideo(args) {
        if (!args?.inputPath) {
    ...
    async handleSwapOptimizedFile(args) {
